      ...
      <div className="grid grid-cols-1 gap-2 min-[420px]:grid-cols-5">
        {RATING_OPTIONS.map((option) => (
          <button
            ...
            className={cn(
              "min-h-14 rounded-2xl border px-3 py-2 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#9AAAD0] focus-visible:ring-offset-2 min-[420px]:text-center",
              option.value === value
          ...
          >
            <span className="flex items-center justify-between gap-2 min-[420px]:block">
              <span className="text-base font-black tabular-nums">{option.label}/5</span>
              <span className={cn("text-xs font-black leading-tight min-[420px]:mt-1 min-[420px]:block", option.value === value ? "text-white" : "text-[#64748B]")}>{option.title}</span>
            </span>
          </button>
        ))}
      </div>
      <div className="rounded-2xl border border-[#DDE6F7] bg-white p-3 shadow-sm">
        <div className="flex flex-col gap-1 min-[460px]:flex-row min-[460px]:items-center min-[460px]:justify-between">
          <p className="text-sm font-black text-[#111B4D]">{selected.title}</p>
          <span className="w-fit rounded-full bg-white px-2.5 py-1 text-xs font-black text-[#111B4D] ring-1 ring-[#DDE6F7]">
            {selected.tone}
          </span>
        </div>
