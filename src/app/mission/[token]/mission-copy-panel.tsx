  ...
  return (
    <Card className="border-blue-100 bg-white/92 shadow-sm">
      ...
      <CardContent className="space-y-4">
        <div className="max-h-64 overflow-auto rounded-3xl border border-violet-100 bg-violet-50/35 p-4 text-sm leading-relaxed text-foreground">
          <pre className="whitespace-pre-wrap font-sans">{missionSummary}</pre>
        </div>

        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
          <Button type="button" variant="outline" className="h-11 justify-start rounded-2xl" onClick={copyMissionSummary}>
            <ClipboardCopy className="mr-2 h-4 w-4" />
            Copier mission
          </Button>
          <Button type="button" variant="outline" className="h-11 justify-start rounded-2xl" onClick={() => copyText(clientMessage, "Message client copié.")}>
            <Phone className="mr-2 h-4 w-4" />
            Copier client
          </Button>
          <Button type="button" variant="outline" className="h-11 justify-start rounded-2xl" onClick={() => copyText(supportMessage, "Message admin copié.")}>
            <Link2 className="mr-2 h-4 w-4" />
            Copier admin
          </Button>
          {clientWhatsAppUrl ? (
            <Button asChild variant="outline" className="h-11 justify-start rounded-2xl border-blue-100 text-blue-800 hover:bg-blue-50">
            ...
            <Button type="button" variant="outline" className="h-11 justify-start rounded-2xl" disabled>
              <MessageCircle className="mr-2 h-4 w-4" />
