...
export function GiftRoad({ currentStep, cycle, cycleEnabled, steps = DEFAULT_LOYALTY_GIFT_STEPS }: GiftRoadProps) {
  ...
  return (
    <section className="gift-road-stage" aria-label="Route animée des cadeaux Compétence" data-gift-road>
      ...
      <style jsx global>{`
        .gift-road-stage { position: relative; min-height: 1000px; overflow: hidden; border: 1px solid #dfe6f2; border-radius: 28px; background: #fff; isolation: isolate; }
        ...
        .gift-road-start { position:absolute; z-index:4; left:50%; top:1.5%; transform:translateX(-50%); border:1px solid #d8dee9; border-radius:999px; background:#fff; color:#111b4d; padding:.55rem .9rem; font-size:.7rem; font-weight:900; text-transform:uppercase; letter-spacing:.08em; box-shadow:0 10px 25px rgba(17,27,77,.12); }
        .gift-road-node { position:absolute; z-index:3; display:flex; align-items:center; gap:.55rem; width:min(185px,42vw); translate:-50% -50%; border:1px solid #dfe6f2; border-radius:18px; background:rgba(255,255,255,.96); padding:.55rem; box-shadow:0 14px 34px rgba(17,27,77,.16); backdrop-filter:blur(10px); }
        ...
        .gift-road-future { position:absolute; z-index:4; left:50%; top:89%; translate:-50% 0; display:flex; align-items:center; gap:.45rem; border:1px solid #e8d7a0; border-radius:999px; background:#fff9e8; color:#6b4f00; padding:.45rem .7rem; font-size:.64rem; font-weight:850; white-space:nowrap; }
        ...
        @media (min-width: 720px) { .gift-road-stage{min-height:1120px}.gift-road-node{width:220px;padding:.7rem}.gift-node-orb{width:48px;height:48px}.gift-node-copy strong{font-size:.9rem}.gift-node-copy small{font-size:.75rem} }
