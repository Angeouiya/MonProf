...
export default function ClientLoading() {
  return (
    <div className="space-y-3" aria-label="Chargement de l'espace client" aria-live="polite">
      <section
        data-client-loading-header
        className="rounded-lg border-b border-[#E6EAF3] bg-white pb-2"
      >
        <div className="mb-2 flex min-h-8 items-center gap-2">
          <Skeleton className="h-9 w-20 bg-white ring-1 ring-[#DDE6F7]" />
          <Skeleton className="h-4 w-24 bg-[#E6EAF3]" />
        </div>
        <Skeleton className="h-7 w-3/4 max-w-xl bg-[#E6EAF3]" />
      </section>
        ...
        <div className="rounded-lg border border-[#E3E8F2] bg-white p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <Skeleton className="mt-3 h-6 w-4/5 bg-[#E6EAF3]" />
            </div>
          </div>
          <div className="mt-4 grid gap-2 min-[520px]:grid-cols-3">
            {[0, 1, 2].map((item) => (
              <Skeleton key={item} className="h-14 bg-white ring-1 ring-[#E3E8F2]" />
            ))}
          </div>
        </div>

        <aside className="hidden rounded-lg border border-[#E3E8F2] bg-white p-4 lg:block">
          <Skeleton className="h-4 w-28 bg-[#E6EAF3]" />
          <Skeleton className="mt-4 h-20 bg-white ring-1 ring-[#E3E8F2]" />
