...
export type ClientCommuneOption = {
  id?: string;
  name: string;
  zone?: string | null;
...
export function ProfileClient({
  initialProfile,
  initialCommuneOptions,
}: {
  initialProfile: ClientProfile;
  initialCommuneOptions: ClientCommuneOption[];
}) {
  const [profile, setProfile] = useState<ClientProfile>(initialProfile);
  const [communeOptions, setCommuneOptions] = useState<ClientCommuneOption[]>(() => (
    initialCommuneOptions.length > 0 ? initialCommuneOptions : COMMUNES.map((name) => ({ name }))
  ));
  ...
  useEffect(() => {
    if (initialCommuneOptions.length > 0) return;
