...
function QuarterManager({ commune, request, onChanged }: { commune: Commune; request: (url: string, method: string, body?: unknown) => Promise<any>; onChanged: () => void }) {
  const [query, setQuery] = useState(""); const [name, setName] = useState(""); const [aliases, setAliases] = useState(""); const [busy, setBusy] = useState(false);
  const filtered = useMemo(() => commune.quarters.filter((item) => `${item.name} ${item.aliases ?? ""}`.toLowerCase().includes(query.toLowerCase())), [commune.quarters, query]);
  const create = async () => { setBusy(true); try { await request(`/api/admin/communes/${commune.id}/quarters`, "POST", { name, aliases: aliases || null, isActive: true }); setName(""); setAliases(""); toast.success("Quartier ajouté"); onChanged(); } catch (error) { toast.error(error instanceof Error ? error.message : "Action impossible."); } finally { setBusy(false); } };
  return <section className="mt-2 border-t border-[#E7ECF5] pt-5"><div className="flex items-start gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-lg bg-[#111B4D] text-white"><MapPin className="h-4 w-4" /></span><div><h3 className="text-sm font-semibold text-[#111827]">Quartiers ({commune.quarters.length})</h3><p className="mt-1 text-xs text-[#64748B]">Les alias reconnaissent les variantes d'écriture sans créer de doublon et facilitent la recherche client du nom officiel.</p></div></div><div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto]"><Input value={name} onChange={(event) => setName(event.target.value)} placeholder="Nom officiel du quartier" /><Input value={aliases} onChange={(event) => setAliases(event.target.value)} placeholder="Alias séparés par des virgules" /><Button onClick={create} disabled={busy || name.trim().length < 2} className="bg-[#111B4D] text-white hover:bg-[#0B143D] sm:col-span-2 lg:col-span-1">{busy ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Plus className="mr-2 h-4 w-4" />}Ajouter</Button></div><div className="relative mt-3"><Search className="absolute left-3 top-3.5 h-4 w-4 text-[#64748B]" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Rechercher un quartier ou un alias" className="pl-9" /></div><div className="mt-3 max-h-80 space-y-2 overflow-y-auto pr-1">{filtered.length ? filtered.map((quarter) => <QuarterRow key={quarter.id} communeId={commune.id} quarter={quarter} request={request} onChanged={onChanged} />) : <p className="rounded-lg border border-dashed border-[#DDE6F7] p-4 text-center text-xs text-[#64748B]">Aucun quartier trouvé.</p>}</div></section>;
}
...
function QuarterRow({ communeId, quarter, request, onChanged }: { communeId: string; quarter: Quarter; request: (url: string, method: string, body?: unknown) => Promise<any>; onChanged: () => void }) {
  const [name, setName] = useState(quarter.name); const [aliases, setAliases] = useState(quarter.aliases ?? ""); const [busy, setBusy] = useState(false); const base = `/api/admin/communes/${communeId}/quarters/${quarter.id}`;
  ...
  const unchanged = name === quarter.name && aliases === (quarter.aliases ?? "");
  return <div className="grid gap-2 rounded-lg border border-[#DDE6F7] p-2 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)_auto]"><Input value={name} onChange={(event) => setName(event.target.value)} aria-label="Nom officiel du quartier" className={quarter.isActive ? "" : "text-[#94A3B8] line-through"} /><Input value={aliases} onChange={(event) => setAliases(event.target.value)} aria-label="Alias du quartier" placeholder="Alias, variantes" /><div className="flex justify-end gap-1"><Button variant="outline" size="icon" disabled={busy || name.trim().length < 2 || unchanged} onClick={() => act("PATCH", { name, aliases: aliases || null })} aria-label="Enregistrer le quartier"><Save className="h-4 w-4" /></Button><Button variant="outline" size="icon" disabled={busy} onClick={() => act("PATCH", { isActive: !quarter.isActive })} aria-label={quarter.isActive ? "Désactiver" : "Activer"}><ToggleLeft className="h-4 w-4" /></Button><Button variant="outline" size="icon" disabled={busy} onClick={() => act("DELETE")} className="text-red-600" aria-label="Supprimer"><Trash2 className="h-4 w-4" /></Button></div></div>;
}
