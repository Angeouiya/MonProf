    ...
    await db.$transaction(async (tx) => {
      ...
      await tx.teacherNotification.create({
        data: {
          ...
          message: `Bonjour ${teacherName}, ${message} pour la réservation ${booking.reference}. Sans retour, le service client pourra envisager un remplacement.`,
          channel: "WHATSAPP",
          sent: false,
          status: "PENDING",
    ...
    await db.$transaction(async (tx) => {
      ...
      await tx.teacherNotification.create({
        data: {
          ...
          message: `Bonjour ${teacherName}, votre confirmation est attendue pour la mission ${mission.booking.reference}. Sans retour rapide, le service client pourra recommander un remplacement.`,
          channel: "WHATSAPP",
          sent: false,
          status: "PENDING",
          sentById: actor.adminId ?? undefined,
    ...
    await db.$transaction(async (tx) => {
      ...
      await tx.teacherNotification.create({
        data: {
          ...
          message: `Bonjour ${teacherName}, votre cours ${booking.reference} est prévu ${scheduleLabel}. Merci de confirmer immédiatement votre disponibilité auprès du service client Compétence.`,
          channel: "WHATSAPP",
          sent: false,
          status: "PENDING",
          sentById: actor.adminId ?? undefined,
