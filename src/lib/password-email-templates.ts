...
export function passwordResetEmailTemplate(input: PasswordResetTemplateInput) {
  ...
  return {
    text: [
      `Bonjour ${name},`,
      "",
      "Une demande de réinitialisation du mot de passe de votre compte Compétence.CI a été reçue.",
    ...
    html: emailShell({
      preview: "Votre lien sécurisé de réinitialisation Compétence.CI",
      content: `
        ...
        <p style="margin:0 0 22px;font-size:15px;line-height:1.7;color:#475569">Une demande de réinitialisation a été reçue pour votre compte Compétence.CI. Cliquez sur le bouton ci-dessous pour continuer.</p>
...
export function passwordChangedEmailTemplate(input: PasswordChangedTemplateInput) {
  const name = input.name.trim() || "utilisateur";
  const accountLabel = input.accountLabel?.trim() || "compte Compétence.CI";
  const safeName = escapeHtml(name);
  ...
  return {
    text: [
      ...
      input.securityUrl,
      "",
      "Le service client Compétence.CI ne vous demandera jamais votre mot de passe par email, SMS ou WhatsApp.",
      "",
      "Compétence.CI",
    ].join("\n"),
    html: emailShell({
      preview: "Confirmation de modification de votre mot de passe Compétence.CI",
      content: `
        <p style="margin:0 0 16px;font-size:16px;line-height:1.6;color:#111827">Bonjour ${safeName},</p>
        ...
        <div style="border:1px solid #fecaca;border-radius:8px;background:#fffafa;padding:16px">
          <p style="margin:0 0 12px;font-size:14px;line-height:1.6;color:#991b1b"><strong>Vous ne reconnaissez pas cette action ?</strong><br>Sécurisez immédiatement votre compte et contactez Compétence.CI si nécessaire.</p>
        ...
        <p style="margin:20px 0 0;font-size:12px;line-height:1.65;color:#64748b">Compétence.CI ne vous demandera jamais votre mot de passe par email, SMS ou WhatsApp.</p>`,
    }),
  };
}
        ...
        <div style="max-width:620px;margin:0 auto;padding:28px 14px">
          <div style="overflow:hidden;border:1px solid #dbe3f0;border-radius:12px;background:#ffffff">
            <div style="border-bottom:4px solid #f4c430;background:#111b4d;padding:22px 24px;color:#ffffff">
              <div style="font-size:21px;font-weight:800;letter-spacing:.2px">Compétence.CI</div>
            ...
            <div style="border-top:1px solid #e7ecf3;background:#f8fafc;padding:15px 24px;font-size:11px;line-height:1.6;color:#64748b">
              Message automatique de sécurité envoyé par Compétence.CI. Ne transmettez jamais vos liens ou mots de passe.
            </div>
          </div>
        </div>
      </body>
