...
export function getClientTransactionStatusLabel(type: string, status: string) {
  ...
  if (type === "RESCHEDULE_FEE") {
    if (status === "BLOCKED") return "Supplément sécurisé";
    if (status === "REFUND_PENDING") return "Supplément à rembourser";
    if (status === "REFUNDED") return "Supplément remboursé";
    return "Supplément créneau";
  }
