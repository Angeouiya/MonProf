...
Après stockage, créer un déploiement Production staged, puis tester le parcours réel « mot de passe oublié » d'un client sur cette URL. Ce test runtime est la porte obligatoire avant promotion : il prouve que le nouveau déploiement a reçu la variable `Sensitive`, que l'outbox fonctionne et que Gmail livre le lien Compétence. Une modification de variable Vercel ne s'applique jamais aux déploiements antérieurs.
...
Ne jamais renseigner ni transmettre le mot de passe Gmail dans le projet, GitHub ou Vercel. Seuls le client OAuth, son secret et le refresh token sont utilisés. `/api/health` expose séparément `configured`, `runtimeEnabled` et `liveVerification: "not_checked_by_health"`; il ne contacte jamais Google et ne prouve donc pas qu'OAuth est opérationnel. Le bootstrap valide l'identité, les scopes et le refresh token avant stockage; le parcours client sur le déploiement staged valide ensuite l'envoi runtime avant promotion. La commande `npm run verify:gmail-live -- --send-self` reste un test local volontaire lorsqu'un jeton est explicitement disponible. La réinitialisation publique par email est réservée aux clients. En cas d'oubli professeur, le service client attribue un mot de passe temporaire depuis la fiche professeur; la prochaine connexion oblige le professeur à le remplacer. Les administrateurs utilisent leur circuit interne géré par le propriétaire.
