...
record(
  ...
  TEACHER_SCHEDULE_CONFLICT_CODE === "TEACHER_SLOT_ALREADY_RESERVED"
    ...
    && /durationMinutes:\s*60\s*\|\s*120/.test(bookingForm)
    && /validateCustomScheduleTime/.test(bookingForm)
    && /aria-disabled=\{Boolean\(occupiedConflict\)\}/.test(bookingForm)
    && /Déjà payé/.test(bookingForm)
    && /Déplacement insuffisant/.test(bookingForm)
    && /selectedScheduleOccurrences[\s\S]*?selectedScheduleConflicts/.test(bookingForm)
    && /scheduleOccurrences:\s*scheduleRows\.map/.test(bookingForm)
    ...
    && /usesExplicitSchedule/.test(bookingCreateApi)
    && /scheduleOccurrences:\s*usesExplicitSchedule/.test(bookingCreateApi)
